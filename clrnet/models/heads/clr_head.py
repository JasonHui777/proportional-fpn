...
class CLRHead(nn.Module):
    ...
    def loss(self,
        ...
        #add
        img_name = [meta_item['full_img_path'] for meta_item in batch['meta']]
        name_loss = {'name': '',
                    'loss1': '',
                    'loss2': '',
                    'loss3': ''}
        all_name_liou_loss = [name_loss for _ in range(len(batch['meta']))]
        all_name_liou_loss = [{'name': img_name, 'loss1': '', 'loss2': '', 'loss3': ''} for img_name, d in zip(img_name, all_name_liou_loss)]
        for stage in range(self.refine_layers):
            ...
            for predictions, target in zip(predictions_list, targets):
                ...
                #print('iou_loss')
                line_louloss = liou_loss(
                   reg_pred, reg_targets,
                    self.img_w, length=15)
                #print(line_louloss)
                iou_list.append(line_louloss)
                iou_loss = iou_loss + line_louloss
        ...
        #     if stage == 0:
        #         for item, c_value in zip(all_name_liou_loss, iou_list):
        #             item['loss1'] = c_value
        #     if stage == 1:
        #         for item, c_value in zip(all_name_liou_loss, iou_list):
        #             item['loss2'] = c_value
        ...
        #         for item, c_value in zip(all_name_liou_loss, iou_list):
        #             item['loss3'] = c_value
        # for d in all_name_liou_loss:
        #     d['loss1'] = f'{d["loss1"].item():.4f}'
        #     d['loss2'] = f'{d["loss2"].item():.4f}'
        #     d['loss3'] = f'{d["loss3"].item():.4f}'


        # csv_files = [f for f in os.listdir('/home/lucky/CLRNet-main/data/result') if f.endswith('.csv')]
        # if not csv_files:
        #     next_filename = '1.csv'
        # else:
        #     last_filename = csv_files[-1]
        #     last_number = int(last_filename.split('.')[0])
        #     next_number = last_number + 1
        #     next_filename = str(next_number) + '.csv'
        #
        # fields = ['name', 'loss1', 'loss2', 'loss3']
        # with open(os.path.join('/home/lucky/CLRNet-main/data/result', next_filename), 'w', newline='') as csvfile:
        #     writer = csv.DictWriter(csvfile, fieldnames=fields)
        #     writer.writeheader()
        #     for data in all_name_liou_loss:
